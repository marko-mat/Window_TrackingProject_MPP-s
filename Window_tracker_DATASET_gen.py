...
def write_to_csv(window_times, session_id, filename='window_time_tracking.csv'):
    file_exists = False
    try:
    ...
    with open(filename, mode='a', newline='') as file:
        ...
        if not file_exists:
            writer.writerow(['Session ID', 'Application', 'Total Time (seconds)'])

                                          # write with session id
        for window, total_time in window_times.items():
            writer.writerow([session_id, window, f'{total_time:.2f}'])

    print(f"\nTime tracking data saved to {filename} for session {session_id}")
...
def monitor_active_window():
    ...
    try:
        ...
        write_to_csv(window_times, session_id)
